...
#		sprite.set_rot(Vector2(-100,0).angle_to_point(get_linear_velocity())+deg2rad(90))
		...
		if vel < 500:
			sprite.set_rotd(lerp(sprite.get_rotd(), 25.0, 20.0*delta))
		elif vel > 500:
			sprite.set_rotd(lerp(sprite.get_rotd(), -90.0, 5.0*delta))

func jump():
	if not is_sleeping() and current_scene.is_playing():
		if (get_pos().y > 0):#gapping the jump height to net over the max height
			set_linear_velocity(Vector2(0,-400))
